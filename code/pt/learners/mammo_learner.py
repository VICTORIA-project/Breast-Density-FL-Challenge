...
import torch
from pt.utils.ResNet50 import ResNet50
...
class MammoLearner(Learner):
    ...
    def initialize(self, parts: dict, fl_ctx: FLContext):
        ...
        self.model = ResNet50()
        num_ftrs = self.model.model.fc.in_features
        self.model.model.fc = CoralLayer(size_in=num_ftrs, num_classes=4)
    ...
    def local_train(
            self, fl_ctx, train_loader, abort_signal: Signal, val_freq: int = 0, global_weights=None
    ):
        for epoch in range(self.aggregation_epochs):
            ...
            for i, batch_data in enumerate(train_loader):
                ...
                # TODO for disabling fedprox mu=0
                mu = 0.01
                ...
                for var_name in local_var_dict:
                    if var_name in model_keys:
                        w = local_var_dict[var_name]
                        w_t = torch.as_tensor(global_weights[var_name], device=self.device)
                        proximal_term += (w - w_t).float().norm(2)

                # loss = self.criterion(outputs, labels) + (mu / 2) * proximal_term

                levels = levels_from_labelbatch(labels,
                                                num_classes=4)
                levels = levels.to(self.device)
                loss = loss_coral(outputs, levels) + (mu / 2) * proximal_term
    ...
    def train(
        ...
        global_weights_copy = copy.deepcopy(global_weights)
        ...
        self.local_train(
            ...
            val_freq=self.val_freq, global_weights=global_weights_copy
        )
        if abort_signal.triggered:
            return make_reply(ReturnCode.TASK_ABORTED)
    ...
    def local_valid(
        ...
        with torch.no_grad():
            ...
            for i, batch_data in enumerate(valid_loader):
                ...
                # TODO check it is working correctly here
                # outputs = torch.softmax(self.model(inputs), dim=1)
                logits = self.model(inputs)
                # outputs = label_from_logits(logits)

                probas = torch.sigmoid(logits)
                # probability
                outputs = torch.cumprod(probas, dim=1)
                num_classes = 4
                if True:
                    # hard coding probs
                    probs = torch.zeros((outputs.size(0), num_classes), dtype=torch.float32)
                    probs[:, 0] = 1 - outputs[:, 0]
                    for l in range(1, num_classes - 1):
                        probs[:, l] = outputs[:, l - 1] - outputs[:, l]
                    probs[:, num_classes - 1] = outputs[:, num_classes - 2]
                else:
                    # CDF probs according to paper
                ...
                if not return_probs_only:
                    ...
                    if True:
                        # getting prediciton normally
                        _pred_label = torch.argmax(probs, dim=1).to(self.device).float()
                    else:
                        # getting prediction according to the paper
                        predict_levels = probs > 0.5
                        _pred_label = torch.sum(predict_levels, dim=1)
